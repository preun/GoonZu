...
HRESULT smallHPPotion::init()
{
	...
	*/
	...
	_item.itemImage = IMAGEMANAGER->addImage("십전대보탕", "./image/equip/potion/hea50.bmp", 34, 34, true, RGB(255, 0, 255));

	_item.limit[LEVEL_INDEX] = 0;		//레벨제한
	_item.limit[STAT_INDEX] = 0;		//스탯제한
	_item.limit[SKILL_INDEX] = 0;		//기술제한

	_item.effect[EFFECT_POTION] = 100;		//방어구 방어력
	_item.effect[EFFECT_MAXATT] = 0;	//

	_item.money = 150;	//상점 500원판매
