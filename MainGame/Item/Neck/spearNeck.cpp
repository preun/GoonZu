...
HRESULT spearNeck::init()
{
	...
	*/
	...
	_item.itemImage = IMAGEMANAGER->addImage("창술목걸이", "./image/equip/neck/nck60.bmp", 34, 34, true, RGB(255, 0, 255));

	_item.limit[LEVEL_INDEX] = 10;		//레벨제한
	_item.limit[STAT_INDEX] = 0;		//스탯제한
	_item.limit[SKILL_INDEX] = 0;		//기술제한

	_item.effect[EFFECT_NECK] = 2;		//목걸이 기술증가량
	_item.effect[EFFECT_MAXATT] = 0;	//

	_item.money = 5000;	//상점 500원판매
